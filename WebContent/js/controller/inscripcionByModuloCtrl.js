app
		.controller(
				...
				function($scope, $http, $resource, $location, sharedProperties,
					...
					$scope.nbActividadModificar = tipoCtrl.actividad=='none'?'':tipoCtrl.actividad;
					$scope.nbHorario = tipoCtrl.nbHorario;
					$scope.nbHorarioModificar = tipoCtrl.nbHorario=='none'?'':tipoCtrl.nbHorario;
					...
					$scope.defaultValuesCheck = function() {
						if ($scope.estadisticasByModuloDto.swFeDesde) {
							...
							if (angular.isDate($scope.estadisticasByModuloDto.feDesde)){
								$scope.estadisticasByModuloDto.swFeDesde = false;
								$scope.estadisticasByModuloDto.feHasta = null;								
							}
						}
						if ($scope.estadisticasByModuloDto.swFeHasta) {
							if (angular.isDate($scope.estadisticasByModuloDto.feHasta)){
					...
					$scope.today = function() {
						...
						if (validaFechaClick()) {
							$scope.graficar();
							/**Buscamos los miembros y lo dibujamos en una tabla*/
							//$scope.miembros = {};
							//$scope.searchMiembros();
						}
					...
					$scope.graficar = function graficar() {
						/** *Buscamos atencion****Grafico******** */
							miembroService.estadisticasGetMiembroChart(
											{
											...
											function success(data, status) {
												chartDto = data.chartDto;
												$scope.miembros = data.miembroDtos;
											...
											function err(httpResponse) {
												$scope.status = status;
												$scope.aviso = 'Ha pasado algo inesperado inscripcionByModuloCtrl.js->miembroService.estadisticasByModulo';
											});
					}
